...
documented in specification section 30.21; exceeding a limit produces an error.

Statuses are **accepted**, **accepted with exact restriction**, **rejected in
0.2**, and **deferred extension**. Deferred forms are rejected by the current
...
| Support imports | accepted with exact restriction | `from typing import NamedTuple` and `from typing import Final`. |
| Plain, relative, star or aliased imports | rejected in 0.2 | Import defining symbols directly. |
| Local, conditional or dynamic imports | rejected in 0.2 | Imports are top-level declarations. |
| `__future__` imports | rejected in 0.2 | Source semantics are selected by configuration. |
...
| Module rebinding or executable statements | rejected in 0.2 | This includes script guards and ordinary calls during initialization. |
| Namespace packages, ambiguous modules, symlinks | rejected in 0.2 | Discovery assigns one canonical name per source file; it never follows source symlinks. |
...
| Default arguments, `/`, bare `*`, `*args`, `**kwargs` | rejected in 0.2 | Signatures contain only ordinary explicitly annotated parameters. |
| Function decorators | rejected in 0.2 | No entrypoint, purity or unsafe decorators. Configure entrypoints by qualified name. |
| Type parameters | accepted with exact restriction | Rank-one unbounded data parameters on pure synchronous top-level functions and records; no overloads. |
| Nested functions | accepted with exact restriction | Pure synchronous monomorphic definitions with stable immutable captures; no definitions in loops or local classes. |
| `class` | accepted with exact restriction | Top-level NamedTuple records with exactly the standard base, immutable fields, and no methods, defaults, or class options. |
| Class decorators | rejected | Standard NamedTuple bases need no decorator. |
...
| Chained or destructuring assignment | rejected in 0.2 | Use a separate simple assignment for each name. |
| Attribute, index or starred assignment target | rejected in 0.2 | No object mutation or unpacking. |
| Imported-name assignment | rejected in 0.2 | Imported declarations are fixed. |
| Capability/host-reference alias or rebinding | rejected in 0.2 | Forward the original parameter directly to an exact known call parameter. |
| Augmented assignment (`+=`, etc.) | rejected in 0.2 | In-place operator semantics are unavailable. |
...
| `assert` | rejected in 0.2 | Represent expected failure as an explicit result. |
| `raise` | rejected in 0.2 | Expected failures are Pure Values. Deterministic primitive failures may terminate an invocation. |
| `try`, `except`, `except*`, exception `else`, `finally` | rejected in 0.2 | No exception inspection, cancellation suppression or cleanup inside verified code. |
...
| `match` / `case` | rejected in 0.2 | Use explicit conditions and optional values. |
| `global`, `nonlocal` | rejected in 0.2 | No ambient mutable state. |
| `del` | rejected in 0.2 | No deletion of names, attributes or subscriptions. |
| Type-alias statement (`type T = ...`) | accepted with exact restriction | Data aliases, optionally generic; cycles must pass through a named record. |
...
| Complex literals | deferred extension | Complex numbers have no 0.2 type. |
| Ellipsis (`...`) value | rejected in 0.2 | Ellipsis appears only in `tuple[T, ...]` annotations. |
| Tuple display | accepted with exact restriction | Homogeneous immutable sequences, or products with an explicit contextual annotation. Empty sequences require an element annotation. |
| List, dict and set displays | rejected in 0.2 | No mutable or unordered containers. |
| List, dict and set comprehensions | rejected in 0.2 | Write explicit loops over approved immutable iterables. |
| Generator expression | deferred extension | First-class suspended computation is unavailable. |
| Name reference | accepted with exact restriction | Assigned immutable data, constants, or pure function values with known provenance. |
...
| Method, callable object, computed call | rejected | Call named functions or tracked local pure callables. |
| Starred expression / argument unpacking | rejected in 0.2 | No `*values` or `**values` expansion. |
...
| Non-call await or await of a sync function | rejected in 0.2 | No first-class awaitables or protocol dispatch. |
| Unawaited async call | rejected in 0.2 | Coroutines cannot be assigned, returned, stored, compared or discarded. |
| `yield`, `yield from` | deferred extension | Generators, send/close and suspended lifetime semantics are unspecified. |
| Lambda | rejected | Use an annotated nested pure function with stable captures. |
| Assignment expression (`:=`) | rejected in 0.2 | Assignment is a statement with a single local target. |
...
| Template string (`t"..."`) | rejected in 0.2 | Template objects and their runtime behavior are outside the closed type model. |
| Reflection, dynamic evaluation and dunder access | rejected in 0.2 | Includes `eval`, `exec`, `compile`, `getattr`, `globals`, `id`, `hash` and runtime type inspection. |
...
| `tuple[T, ...]` | accepted with exact restriction | Immutable data or a data type parameter element type. |
| `T | None` | accepted with exact restriction | Immutable data or a data type parameter `T`; no arbitrary union or subtyping. |
...
| Quoted annotations, `Any`, `object`, protocols, mutable container types | rejected | No arbitrary annotation execution or protocol dispatch. |
| `tuple[A, B]` | accepted with exact restriction | Immutable product; access uses a constant in-range index. |
| `Callable[[A], B]` | accepted with exact restriction | Pure synchronous function; tracked provenance; no host callback entrypoints. |
| Generic records and aliases | accepted with exact restriction | Explicit data type arguments in annotations; regular recursion only. |
...
| --- | --- |
| `len` | One `str`, `bytes`, homogeneous tuple, or product; returns `int`. |
| `ord`, `chr` | One `str` to `int`, or one `int` to `str`; Python domain exceptions apply. |
| `copy.replace` | Exact verified record and explicit distinct field keywords; returns the same record type. |
...
Construction and grouping use immutable records, folds, and persistent structures.
No mutable builder or PurePy collection runtime is supplied. See [Functional core](FUNCTIONAL_CORE.md).
